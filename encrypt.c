...
int encrypt() {
	...
	uint32_t H2F, F2H;
	void *h2p_lw_h2f_addr, h2p_lw_f2h_addr;
	...
	h2p_lw_h2f_addr=virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + PIO_CHAOS_SHIFT_BASE ) & ( unsigned long)( HW_REGS_MASK ) );
	h2p_lw_f2h_addr=virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + PIO_CHAOS_TEMP_BASE ) & ( unsigned long)( HW_REGS_MASK ) );
	printf("h2p_lw_h2f_addr = %x\n",h2p_lw_h2f_addr);
	printf("h2p_lw_f2h_addr = %x\n",h2p_lw_h2f_addr);
	H2F = *(uint32_t *)h2p_lw_h2f_addr;
	F2H = *(uint32_t *)h2p_lw_f2h_addr;
	printf("H2F = %d",H2F);
	printf("F2H = %d",F2H);
